import os
import subprocess
from fetch_times import binary_search_frame

def process_vid(folder_path, output_path, filename):
  ...
  ext_cam = os.path.join(folder_path, files[0])
  ...
  cam1 = os.path.join(folder_path, files[0])
  cam2 = os.path.join(folder_path, files[1])
  cam3 = os.path.join(folder_path, files[2])
  cam4 = os.path.join(folder_path, files[3])

  output_file = f"{output_path}/{filename}.mp4"
  ...
  stack_start_sec = stack_start
  stack_end_sec   = stack_end
  ...
  filter_complex = (
    # 1. Process Camera 4
    f"[3:v]crop=ih*9/16:ih:(iw-ow)/2:0,scale=1080:1920:flags=lanczos,eq=brightness={BRIGHTNESS}:contrast={CONTRAST}:saturation={SATURATION}[cam4_processed];"

    # 2. Process Camera 1, 2, 3
    f"[0:v]scale=iw*0.68:ih*0.68:flags=lanczos,eq=brightness={BRIGHTNESS}:contrast={CONTRAST}:saturation={SATURATION}[cam1_scaled];"
    f"[1:v]crop=iw:ih*(1-0.15-0.28):0:ih*0.15,scale=iw*0.72:ih*0.72:flags=lanczos,eq=brightness={BRIGHTNESS}:contrast={CONTRAST}:saturation={SATURATION}[cam2_scaled];"
    f"[2:v]scale=iw*0.47:ih*0.47:flags=lanczos,eq=brightness={BRIGHTNESS}:contrast={CONTRAST}:saturation={SATURATION}[cam3_scaled];"
  ...
  command = [
    ...
    '-ss', str(cam4_start),  '-to', str(cam4_end),  '-i', str(cam4),
